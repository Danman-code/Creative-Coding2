...
let score = 0;
let munchSound, blehSound, bgMusic;
let startTime;
let gameOver = false;
...
// sounds
  munchSound = loadSound('Eating sound effectsFree Download & No Copyright.mp3');
  blehSound = loadSound('Bleh Sound Effects.mp3');
  bgMusic = loadSound('CoconutMall.mp3');
}
function setup() {
  ...
  // 5 Food objects
  foods.push(new Food(120, 180, 70, alienBottom, false));
  foods.push(new Food(280, 320, 65, alienTop, false));
  foods.push(new Food(450, 140, 50, cherry, true));
  foods.push(new Food(620, 400, 75, coconut, true));
  foods.push(new Food(180, 480, 60, orangeImg, true));
}
function draw() {
  // Calculate time left
  ...
  if (!gameOver) {
    ...
    for (let food of foods) {
      ...
      if (d < (character.w / 2 + food.size / 2)) {
        if (food.isGood) {
          score++;
          munchSound.play();
        } else {
...
class Food {
  constructor(x, y, size, img, isGood) {
    this.x = x;
    this.y = y;
    this.size = size;
    this.img = img;
  ...
  pickRandomTarget(avoidCX = -999, avoidCY = -999) {
    ...
    } while (avoidCX > 0 && sqrt((this.targetX - avoidCX)**2 + (this.targetY - avoidCY)**2) < 120);
